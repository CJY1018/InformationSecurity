...
public class MyDES {
    ...
    */
    ...
    public static byte[] encryptECB(String text, String key) {
        StringBuilder oriText = new StringBuilder(text);
        int remainder = oriText.length() % 8;

        // 不满足 8 的倍数时，补 0,余数
        if (remainder != 0) {
            END_ZEROS = 8 - remainder;
            for (int i = 0; i < END_ZEROS; i++) {
                oriText.append("0");
            }
            if(oriText.length() < 80)
                System.out.println("原文不满足 8 的倍数，末尾补充" + END_ZEROS + "个 0，补充后的原文为：" + oriText);
            else
                System.out.println("原文不满足 8 的倍数，末尾补充" + END_ZEROS + "个 0，补充后的原文为：" + oriText.substring(0, 80) + "..." + oriText.substring(oriText.length() - 10));
        }

        oriText = new StringBuilder(convertStringToHex(oriText.toString()));

        // 密钥
        key = convertStringToHex(key);

        byte[] keys = parseBytes(key);
        byte[] texts = parseBytes(oriText.toString());

        byte[] ciphertext = new byte[texts.length];

        // encrypt each 8-byte (64-bit) block of the message.
        for (int i = 0; i < texts.length; i += 8) {
            encryptBlock(texts, i, ciphertext, i, keys);
        }
     ...
     * */
    public static byte[] decryptECB(String result, String key) {
        byte[] message = parseBytes(result);
        key = convertStringToHex(key);
        byte[] keys = parseBytes(key);
        byte[] ciphertext = new byte[message.length];

        // decrypt each 8-byte (64-bit) block of the message.
        for (int i = 0; i < message.length; i += 8) {
            decryptBlock(message, i, ciphertext, i, keys);
        }
    ...
    /*
     * ECB解密方式，明文按照64bit切分，不足64bit填0，去掉末尾的0
     * */
    public static byte[] decryptECBOrigin(String result, String key) {
        byte[] ciphertext = decryptECB(result, key);
        // 去掉末尾的0
        if (END_ZEROS != 0) {
            byte[] temp = new byte[ciphertext.length - END_ZEROS];
    ...
    public static String hex2byte(String decryptedHex) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < decryptedHex.length(); i += 3) {
            String str = decryptedHex.substring(i, i + 2);
            sb.append((char) Integer.parseInt(str, 16));
        }
        //将Ú字符替换为换行符
        return sb.toString().replace("Ú", "\r\n");
    }
